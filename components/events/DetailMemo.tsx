    ...
    <View style={[styles.container, expanded && styles.expanded]}>
      <View style={styles.header}>
        <Pressable accessibilityRole="button" onPress={expand} style={styles.heading}>
          <FontAwesomeFreeSolid name="pen" size={16} color={AdaptiveColors.icon} />
      ...
      {expanded && <View style={styles.tagEditor}>
        <TextInput ref={tagInputRef} value={tagInput} onChangeText={setTagInput} onFocus={() => onInputFocus?.(tagInputRef.current)} onBlur={() => onInputBlur?.(tagInputRef.current)} onSubmitEditing={addTag} returnKeyType="done" placeholder="태그 추가..." placeholderTextColor={AdaptiveColors.textMuted} style={styles.tagInput} />
...
const styles = StyleSheet.create({
  ...
  expanded: { borderWidth: 1, borderColor: AdaptiveColors.border, borderRadius: 18, backgroundColor: AdaptiveColors.surface, padding: 18 },
  header: { minHeight: 25, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  heading: { flexDirection: 'row', alignItems: 'center', gap: 10 }, headingText: { color: AdaptiveColors.textSecondary, fontSize: 17, fontWeight: '700' },
  saveButton: { paddingVertical: 4 }, saveText: { color: AdaptiveColors.accent, fontSize: 16, fontWeight: '700' },
  memoInput: { minHeight: 54, borderWidth: 1, borderColor: AdaptiveColors.border, borderRadius: 14, paddingHorizontal: 16, backgroundColor: AdaptiveColors.input, color: AdaptiveColors.text, fontSize: 16 },
  memoTextArea: { minHeight: 150, paddingTop: 14 },
  tagEditor: { flexDirection: 'row', gap: 10 }, tagInput: { flex: 1, height: 50, borderWidth: 1, borderColor: AdaptiveColors.borderStrong, borderRadius: 14, paddingHorizontal: 16, backgroundColor: AdaptiveColors.input, color: AdaptiveColors.text, fontSize: 16 },
  addButton: { minWidth: 72, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: AdaptiveColors.border, borderRadius: 14 }, addDisabled: { opacity: 0.45 }, addText: { color: AdaptiveColors.textSecondary, fontSize: 16, fontWeight: '700' },
  tagList: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, tag: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 22, backgroundColor: AdaptiveColors.backgroundElement, paddingHorizontal: 14, paddingVertical: 9 }, tagText: { color: AdaptiveColors.text, fontSize: 15, fontWeight: '600' }, removeText: { color: AdaptiveColors.icon, fontSize: 22, lineHeight: 18 },
});
