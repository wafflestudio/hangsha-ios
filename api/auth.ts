...
export async function refresh() {
  ...
  const response = await apiClient.post<AuthTokens>("mobile/auth/refresh", {
    refreshToken,
  });
  await TokenService.setTokens(response.data);
  return response.data;
...
export async function establishSession(accessToken: string) {
  const temporaryAccessToken = accessToken.trim();
  if (!temporaryAccessToken) throw new Error("The access token is empty.");
